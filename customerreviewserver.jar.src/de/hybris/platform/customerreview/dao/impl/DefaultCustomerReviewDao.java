...
/*    */   }
		//	get Total Reviews For Product On Inclusive Rating initialRatingRange,finalRatingRange
		public List<CustomerReviewModel> getTotalReviewsForProductOnInclusiveRating(ProductModel product,double initialRatingRange,double finalRatingRange)
		{
			String query = "SELECT {" + Item.PK + "} FROM {" + "CustomerReview" + "} WHERE {" + "product" + "}=?product and  ( rating >= ?initialRatingRange AND rating <= ?finalRatingRange) ORDER BY {" + "creationtime" + "} DESC";
			FlexibleSearchQuery fsQuery = new FlexibleSearchQuery(query);
			fsQuery.addQueryParameter("product", product);
			fsQuery.addQueryParameter("initialRatingRange", initialRatingRange);
			fsQuery.addQueryParameter("finalRatingRange", finalRatingRange);
